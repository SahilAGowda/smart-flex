...
app.post('/register', async (req, res) => {
    ...
    const values = [name, parseInt(age), sex, parseFloat(weight), parseFloat(height), email, password];
...
app.post('/api/nutrition/logs', async (req, res) => {
    const { userId, meal_type, food_items, calories, protein, carbs, fat } = req.body;
    try {
        ...
        const [newLog] = await dbPool.execute('SELECT * FROM nutrition_logs WHERE id = ?', [result.insertId]);
...
app.get('/api/profile/:userId', async (req, res) => {
    try {
        const [rows] = await dbPool.execute('SELECT * FROM users WHERE id = ?', [req.params.userId]);
        if (rows.length > 0) {
    ...
    try {
        ...
        if (workoutSession) {
            ...
            if (workoutData) {
                ...
                response = "I'm having trouble accessing your workout data. Please try again.";
            }
        } else {
...
function generateWorkoutResponse(message, workoutData) {
    ...
    if (lowerMessage.includes('form') || lowerMessage.includes('posture')) {
        ...
        if (workoutData.formScore < 80) {
            formMessage = workoutData.formScore >= 60 ? 
    ...
    } else if (lowerMessage.includes('calories') || lowerMessage.includes('burn')) {
