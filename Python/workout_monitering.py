...
with mp_pose.Pose(min_detection_confidence = 0.5 ,min_tracking_confidence = 0.5) as pose:
    ...
    while cap.isOpened():
        ...
        try:
            ...
            h, w, _ = image.shape
            cv2.putText(image, str(angle), (int(elbow[0] * w), int(elbow[1] * h)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    #    setup Status Box
        ...
        # to adjust the counter variable
        if counter >= 10:
            cv2.putText(image, 'REPS', (5, 60), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2, cv2.LINE_AA)    
            cv2.putText(image,'STAGE',(5,12),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),1,cv2.LINE_AA)
